...
export async function POST(request: NextRequest) {
  try {
    ...
    const { service, token, amount, asset, payer, network, paymentPayload } = body;
    ...
    if (paymentPayload) {
      ...
      // Process the service with refund protection
      if (REFUND_API_KEY) {
        const protectedProcessor = withRefundProtection(
          ...
          async (context) => {
            const result = await processService(context, service);
            return result;
          }
        );
      ...
      const serviceResult = await processService(paymentContext, service);
